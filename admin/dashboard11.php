...
<?php
	...
	$sql_is_to_show_product_views = "select * from app_properties where prop_name='admin_n_rows_visits' ";
...
function getBLdetails(n)
{
	...
	var url = "getBusinessLogLoginsDetails.php";			
        ...
        <nav class="navbar navbar-default navbar-fixed">
            <div class="container-fluid">
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand" href="#">Посещения</a> 
					<a class="navbar-brand" href="dashboard11.php" style="margin-left:10px;"><span class="glyphicon glyphicon-refresh"></span></a>
					<div id="retroclockbox1" style="margin-left:8px; margin-top:-2px;" class="navbar-brand"></div>
					<a class="navbar-brand" href="http://crystalsky.co.il" target='_blank' style="margin-left:10px;">перейти в магазин...</a>
                </div>
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card ">
                            <div class="header">
                                <h4 class="title">Посещения магазина "Crystal Sky"</h4>
                                <p class="category">Посещения магазина "Crystal Sky" за последние 7 дней, представлено в обратном порядке по времени.</p>
                            </div>
                            <div class="content"  style="overflow-x:auto;">   
								...
								$sql0="select count(1) n_of_r from businesslog_logins where datex BETWEEN (NOW() +INTERVAL 10 HOUR- INTERVAL 7 DAY) AND (NOW() +INTERVAL 10 HOUR)";								
								$result0 = $conn->query($sql0);
								$row0 = $result0->fetch_assoc();								
								$total_results = $row0["n_of_r"];									
																								
								$sql = "select * from (select x.*, DATE_FORMAT(x.datex,'%d/%m/%Y %H:%i:%s') as the_date from businesslog_logins x where x.datex between (NOW() +INTERVAL 10 HOUR- INTERVAL 7 DAY) AND (NOW() +INTERVAL 10 HOUR) order by datex desc) M limit ".$p*$n_results_on_page.", ".$n_results_on_page;
								...
								?>	
										...
										<td>
											<button type="button" class="btn btn-success btn-sm" onClick="getBLdetails(<?php echo $row["id"]; ?>);">
												Детали
											</button>										
										</td>
										</tr>											
								...
								<nav>
								  <ul class="pager">
									<li><a href="dashboard11.php?p=<?php echo $first_page; ?>">Начало</a></li>
									<li><a href="dashboard11.php?p=<?php echo $prv_page; ?>">Предыдущая</a></li>
									<li><a href="dashboard11.php?p=<?php echo $p; ?>">Это <?php echo ($p+1); ?> страница</a></li>
									<li><a href="dashboard11.php?p=<?php echo $nxt_page; ?>">Следующая</a></li>
									<li><a href="dashboard11.php?p=<?php echo $last_page; ?>">Конец</a></li>
								  </ul>
								</nav>
								<!-- pager -->
...
<div class="modal fade" id="myModalBusinesslogDetails" tabindex="-1" role="dialog" 
