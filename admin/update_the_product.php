<?php
	...
	if (isset($_POST['makat']) && isset($_POST['title']) && isset($_POST['price']) && isset($_POST['categ']) && isset($_POST['sub_categ'])
		&& isset($_POST['short_desc']) && isset($_POST['long_desc']) && isset($_POST['rowid']))
	{
		...
		$subject = "The updated product from Crystalsky.co.il";	
		...
		$message = '<html><body>Здравствуйте, Crystalsky!' . '<br/><br/> ' ."\n" .
					 ...
					 'ID товара, который был изменен: <a href="http://crystalsky.co.il/item.php?i='.$rowid.'" target="_blank"><b>'.$rowid.'</b></a><br/>' ."\n" .				 
					 'Макат: <b>' . $makat . '</b><br/> ' ."\n" .
					 'Наименование: <b>' . $title . '</b><br/> ' ."\n" .
					 '<br/>С уважением, ' . ' <br/> ' ."\n" .
			...
			{  				
				...
				VALUES ('$db_cur_dt', 'UPDATE_PRODUCT', '$ip_addr', '$admin_email', '$message' )";
				...
				if ($flg_photo2)
				{
					$sql22 = " update products 
								set photo2 = '$photo2'
								where id = ".$rowid;
					
					if ($conn->query($sql22) === TRUE) {
						// echo "4 Record created successfully";
					} else {
						//  echo "Error3: " . $sql22 . "<br>" . $conn->error;
					}		
				}	

				if ($flg_photo3)
				{
					$sql23 = " update products 
								set photo3 = '$photo3'
								where id = ".$rowid;
					
					if ($conn->query($sql23) === TRUE) {
						// echo "5 Record created successfully";
					} else {
						 // echo "Error3: " . $sql23 . "<br>" . $conn->error;
					}		
				}				
