...
<?php
	...
	$sql_is_to_show_product_views = "select * from app_properties where prop_name='admin_n_rows_journal' ";
...
function ReloadPage(selectObject)
{
	 ...
	 window.location.href = 'dashboard2.php?p='+value;
}
...
function getBLdetails(n)
{
	...
	var url = "getBusinessLogDetails.php";			
        ...
        <nav class="navbar navbar-default navbar-fixed">
            <div class="container-fluid">
                <div class="navbar-header">
					...
					<a class="navbar-brand" href="dashboard2.php" style="margin-left:10px;"><span class="glyphicon glyphicon-refresh"></span></a>
			...
			$sql0="select count(1) n_of_r from businesslog where datex BETWEEN (NOW() +INTERVAL 10 HOUR- INTERVAL 90 DAY) AND (NOW() +INTERVAL 10 HOUR)";								
			$result0 = $conn->query($sql0);
			$row0 = $result0->fetch_assoc();								
			$total_results = $row0["n_of_r"];									
																			
			$sql = "select * from (select x.*, DATE_FORMAT(x.datex,'%d/%m/%Y %H:%i:%s') as the_date from businesslog x where x.datex between (NOW() +INTERVAL 10 HOUR- INTERVAL 90 DAY) AND (NOW() +INTERVAL 10 HOUR) order by datex desc) M limit ".$p*$n_results_on_page.", ".$n_results_on_page;
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card ">
                            <div class="header">
                                <h4 class="title">Журнал магазина "Crystal Sky"</h4>
                                <p class="category">Основные ключевые события магазина "Crystal Sky" за последние 90 дней, представлено в обратном порядке по времени.</p>
                            </div>
                            <div class="content"  style="overflow-x:auto;">   
								...
								<nav>
								  <ul class="pager">
									<li><a href="dashboard2.php?p=<?php echo $first_page; ?>">Начало</a></li>
									<li><a href="dashboard2.php?p=<?php echo $prv_page; ?>">Предыдущая</a></li>
									<li><a href="dashboard2.php?p=<?php echo $p; ?>">Это <?php echo ($p+1); ?> страница</a></li>
									<li><a href="dashboard2.php?p=<?php echo $nxt_page; ?>">Следующая</a></li>
									<li><a href="dashboard2.php?p=<?php echo $last_page; ?>">Конец</a></li>
								  </ul>
								</nav>
								<!-- pager -->
